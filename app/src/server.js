...
app.post('/api/orders', (req, res) => {
  ...
  try {
    ...
    console.log(JSON.stringify({ level: 'info', msg: 'Order created', orderId: order.id, customer: order.customer, total: order.total }));
    span.end();
    res.status(201).json(order);
  } catch (err) {
    span.setStatus({ code: SpanStatusCode.ERROR, message: err.message });
    span.recordException(err);
    console.log(JSON.stringify({ level: 'error', msg: 'Order creation failed', error: err.message }));
    span.end();
...
app.put('/api/orders/:id', (req, res) => {
  ...
  console.log(JSON.stringify({ level: 'info', msg: 'Order updated', orderId: order.id, from: oldStatus, to: order.status }));
  span.end();
  res.json(order);
});
...
app.delete('/api/orders/:id', (req, res) => {
  ...
  console.log(JSON.stringify({ level: 'info', msg: 'Order deleted', orderId: removed.id }));
...
app.post('/api/simulate', async (req, res) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    activeOrders.add(1);
    console.log(JSON.stringify({ level: 'info', msg: 'Simulated order', orderId: order.id, customer, item, total }));
  }
